...
//根据时间戳获取表名
func GetTableByTimestamp(tableName string, timestamp int64) string {
	return tableName + time.Unix(timestamp, 0).Format("20060102")
}

//[今天统计昨天的数据专用]获取本月的表名(减去一天,然后计算出昨天的月份)(如果不减去一天，在月份交际之际会计算出错误的月份)
func GetThisMonthTable(tableName string) string {
	return tableName + time.Now().AddDate(0, 0, -1).Format("200601")
}

//获取今天所在的月份表名
func GetTodayMonthTable(tableName string) string {
	return tableName + time.Now().Format("200601")
}
...
//获取今天的表名
func GetTodayTable(tableName string) string {
	return tableName + time.Now().Format("20060102")
}

//获取明天的表
func GetTomorrowTable(tableName string) string {
	return tableName + time.Now().Add(24*time.Hour).Format("20060102")
}

//获取昨天的表
func GetYesterdayTable(tableName string) string {
	d, _ := time.ParseDuration("-24h")
	return tableName + time.Now().Add(d).Format("20060102")
}

//获取两天前的表
func GetTwoDaysBeforeTable(tableName string) string {
...
暂时不支持秒级别，如需支持，please contact me
*/
func GetSpecifyTable(tableName string, specifyTime string) (string, error) {
	d, err := time.ParseDuration(specifyTime)
	if err != nil {
		return "", err
	}

	if strings.Contains(specifyTime, "m") {
		tableName = tableName + time.Now().Add(d).Format("200601021504")
		return tableName, nil
	} else if strings.HasSuffix(specifyTime, "h") {
		num, err := strconv.ParseInt(strings.TrimSuffix(strings.TrimPrefix(specifyTime, "-"),
			"h"), 10, 32)
		if err != nil {
			return "", err
		}
		if num >= 24 {
			tableName = tableName + time.Now().Add(d).Format("20060102")
			return tableName, nil
		} else {
			tableName = tableName + time.Now().Add(d).Format("2006010215")
			return tableName, nil
		}
	}
