...
//初始化任务并加入任务表,默认最大为100个
func AddTask(tname string, spec string, f TaskFunc) *toolbox.Task {
	tk := toolbox.NewTask(tname, spec, toolbox.TaskFunc(f))
	toolbox.AddTask(tname, tk)
	return tk
}

//在正式运行任务之前，测试下任务
func TestTask(task *toolbox.Task) error {
	return task.Run()
}
