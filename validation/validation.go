...
// Identity Test that the obj is mainland of China's id card number if type is string
func (v *Validation) Identity(obj interface{}, key string) *Result {
	return v.apply(Identity{key}, obj)
}
...
// MatchChinese Test that the obj matches chinese name if type is string
// such as "迪丽热巴·迪力木拉提", it pass,but if "迪丽热巴·迪力木拉提12" it fail,that's exactly what i want
func (v *Validation) MatchChinese(obj interface{}, key string) *Result {
	return v.apply(Match{regexp.MustCompile(`^\W+$`), key}, obj)
}
