...
//Identity check th chinese id card number
type Identity struct {
	Key string
}

// IsSatisfied judge whether obj is valid
func (a Identity) IsSatisfied(obj interface{}) bool {
	identity, ok := obj.(string)
	if !ok {
		return false
	}
	ok, err := regexp.MatchString(`^(\d{17})([0-9]|X)$`, identity)
	if err != nil || !ok {
		return false
	}
	...
	var array18 = [...]byte{1, 0, 'X', 9, 8, 7, 6, 5, 4, 3, 2}
	//string to byte
	for k, v := range []byte(identity) {
		idCard[k] = v
		array[k], _ = strconv.Atoi(string(v))
	}

	for i := 0; i < 17; i++ {
		sum += array[i] * wi[i]
	}
	if array18[sum%11] != byte2byte(idCard[17]) {
		return false
	}
	return true
}
func byte2byte(x byte) byte {
	if x == 88 {
		return 'X'
	}
	return x - 48
...
// DefaultMessage return the default Identity error message
func (a Identity) DefaultMessage() string {
	return MessageTmpls["Identity"]
}

// GetKey return the m.Key
func (a Identity) GetKey() string {
	return a.Key
}

// GetLimitValue return the limit value
func (a Identity) GetLimitValue() interface{} {
	return nil
}
...
var mobilePattern = regexp.MustCompile(`^((\+86)|(86))?(1(([35789][0-9])|[8][0-9]|[7][01356789]|[4][579]))\d{8}$`)
